...
export default function CollectibleDetail() {
  ...
  useEffect(() => {
    ...
    const getInfo = async () => {
      axios({
        url: "https://loud-final.herokuapp.com/get_collectible_info",
        method: "POST",
        data: {
    ...
    if (window.ethereum) {
      ...
      const send = await window.contract.methods
        ...
        .on("transactionHash", function (hash) {
          ...
          axios({
            method: "POST",
            url: "https://loud-final.herokuapp.com/update_collectible_history",
            data: {
              tokenId: window.location.href.split("/").pop(),
              newOwner: sender,
  ...
  const stopSale = async () => {
    ...
    axios({
      method: "POST",
      url: "https://loud-final.herokuapp.com/stop_collectible_sale",
      data: {
        tokenId: window.location.href.split("/").pop(),
      },
    })
      .then((res) => {
        alert("successfull Sale stopped!");
        setLoading(false);
        window.location.reload();
      })
  ...
  const startSale = async () => {
    ...
    axios({
      ...
      if (res.data.multiple_transfer) {
        axios({
          method: "POST",
          url: "https://loud-final.herokuapp.com/start_collectible_Sale",
          data: {
            tokenId: window.location.href.split("/").pop(),
          },
        })
  ...
  const buyNft = async () => {
    ...
    if (window.ethereum) {
      ...
      let total_price = (
        parseFloat(nftData.item_price) * parseFloat(buyQuantity) +
        0.01
      ).toFixed(3);
      ...
      window.web3.eth
        ...
        .on("receipt", function (receipt) {
          ...
          axios({
            method: "POST",
            url: "https://loud-final.herokuapp.com/transfer_collectible",
            data: {
        ...
        <section className="item-details-area">
            ...
            <div style={{ width: "32vw", height: "32vh" }}>
              {transferLoad ? (
                ...
                <>
                  ...
                  <div className="pt-5">Enter Quantity to Transfer:</div>
                  <input
                    type="text"
                    className="form-control mt-2"
          ...
          >
            {buyLoading ? (
              ...
              <div style={{ width: "32vw", height: "52vh" }}>
                <div
                  style={{
                    fontSize: "28px",
                    display: "flex",
                ...
                <div>
                  ...
                  <div className="pt-5 d-flex justify-content-between">
                    <div> Item price:</div>
                    <div> {nftData.item_price + "  BNB"}</div>
                  </div>
                  ...
                  <div className="item-info-list mt-4">
                    <ul className="list-unstyled">
                      <li className="price d-flex justify-content-between">
                        <span>
                          Current Price: {nftData.item_price + " BNB"}
                        </span>
